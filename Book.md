...
如何在保证去中心化和安全性的同时尽可能提高性能，目前已有一些缓解办法，例如状态通道（最著名的就是闪电网络，即Lightning Network）、侧链（Sidechain）、Plasma、Rollups（包括Optimistic Rollups，简称OP Rollups，以及Zero Knowledge Rollups，简称ZK Rollups）等。这些技术均不改变区块链主网的底层架构与核心规则，而是在主网之外搭建独立的处理层，承担大部分交易计算与数据处理工作，仅将关键结果同步回主网，因此被称为二层网络（Layer Two，简称L2）。由于理解这些技术需要较强的技术背景，这里我们就不展开讨论了。
...
| OP Rollups（Optimistic Rollups）             |                  | 第五章，没有银弹 |
| ZK Rollups（Zero Knowledge Rollups）         |                  | 第五章，没有银弹 |

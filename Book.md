...
为了使整个系统高效运行，QLend采取了动态的利率调整机制。简单来说，利率和借款池内X所占的比例成反比。当借贷池内有大量的X时，系统会适当降低借贷利率，鼓励借款方借出X。而当池内X变少时，系统会提高借贷利率，督促借款方尽快归还X。所以，借贷利率实际上是根据市场供需来调整的。当市场需要更多的X的时候，借款的人自然就会变多，利率就会上涨，反之就会下降。我们把X已借出数量和X总供应数量的比例叫做借贷池的利用率（Utilization Ratio）。总体而言，我们希望一个运作良好的借贷池保持一个较高的资金利用率，例如80%。

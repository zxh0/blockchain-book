...
\end{aligned}
$$
然后这个坏人再把价格降回到原来的`1`，于是坏人获利，我们吃亏蒙受损失。注意，当池子的价格和外面的世界发生偏离时，通过买卖把池子的价格跟外面的价格拉平而获利的行为是有益的，我们把这种行为叫做套利（Arbitrage）。但是这种发现有利可图的交易，然后在前面插入一笔交易的套利行为是有害的，会严重影响用户体验，我们把这种行为叫做抢跑攻击（Front-Running Attack）。
